...
Данные читаются напрямую из папки data/ — код дашборда не меняется между
срезами, актуальный срез подхватывается автоматически (см. load_long_table).
"""
...
def load_long_table() -> tuple[pd.DataFrame, str]:
    """Длинная таблица датирована по срезам (raw-архив сохраняется целиком) —
    берём самый свежий файл admissions_long_*.parquet по имени."""
    files = sorted(DATA_DIR.glob("admissions_long_*.parquet"))
    if not files:
        st.error("Не найдено ни одного data/admissions_long_*.parquet")
        st.stop()
    path = files[-1]
    df = pd.read_parquet(path)
    snapshot = path.stem.replace("admissions_long_", "")
    return df, snapshot
...
def load_metrics():
    ...
    main = pd.read_csv(
        DATA_DIR / "campaign_metrics_main.csv",
        dtype={"budget_places": "Int64", "commercial_places": "Int64"},
    )
    ranked = pd.read_csv(DATA_DIR / "campaign_metrics_m4_desirability_ranked.csv")
    no_places = pd.read_csv(DATA_DIR / "campaign_metrics_no_budget_places.csv")
    meta = json.loads((DATA_DIR / "campaign_metrics_meta.json").read_text(encoding="utf-8"))
    priority_dist = pd.read_csv(DATA_DIR / "metric_priority_distribution.csv")
    inter_budget = pd.read_csv(DATA_DIR / "metric_program_intersections_budget.csv", index_col=0)
    inter_commercial = pd.read_csv(DATA_DIR / "metric_program_intersections_commercial.csv", index_col=0)
