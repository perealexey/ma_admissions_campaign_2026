...
Три разреза (см. промпт задачи, §4.6, + доп. страница сравнения):
  1. По программе — подробные метрики, рейтинг интереса, распределение
     приоритетов, пересечения.
  2. Сравнить программы — две программы рядом, как карточки на маркетплейсе;
     профиль приоритетов и таблица общих абитуриентов (их id + приоритет
     по каждой из двух программ).
...
st.info(
    ...
    "запасным вариантом.\n\n"
    "Данные охватывают конкурс по двум кампусам: Москва, Санкт-Петербург."
)
st.info(
    "**Как пользоваться.**\n\n"
...
with tab_program:
    ...
    if len(prog_students):
        ...
        # Колонки остаются Int64 (не строкой): в Streamlit 1.50.0 клик по шапке
        # таблицы для интерактивной пересортировки сортирует по типу колонки —
        # строковая колонка сортируется лексикографически ("1, 10, 11, 2"), а
        # Int64/float — численно. Цена — пропуски рисуются буквально текстом
        # "None" вместо пустой ячейки (родной баг рендерера этой версии), но
        # это менее важно, чем корректная сортировка (см. CONTEXT_FOR_FUTURE.md).
        st.dataframe(
        ...
        st.caption(f"Всего абитуриентов по этой программе: {len(applicants_table)}. "
                   "«None» в приоритете — не подавался этим типом места.")
    else:
    ...
    if program_a is None or program_b is None:
        ...
        if ambiguous:
            ...
            rows = {
                ...
                "Интерес": cmp["desirability"].round(3),
                "Ранг интереса (из 125)": cmp["desirability_rank"],
            }
            ...
            st.caption(
                ...
                "рейтинга интереса) — не путать с нулевым интересом. «Общие уникальные "
            ...
            if common_ids:
                ...
                # Все колонки — Int64 (не строка): клик по шапке таблицы в
                # Streamlit 1.50.0 пересортировывает по типу колонки, строковая
                # колонка сортируется лексикографически ("1, 10, 11, 2"), а
                # Int64 — численно. Пропуски (не подавался этим видом конкурса)
                # при этом рисуются буквально текстом "None" — известный баг
                # рендерера этой версии, но не важно так, как корректная
                # сортировка (см. CONTEXT_FOR_FUTURE.md).
                ...
                for c in ("a_bud", "a_com", "b_bud", "b_com"):
                    common_table[c] = common_table[c].astype("Int64")
                ...
                st.caption(
                    ...
                    "«None» — не подавался этим видом. Нумерация приоритетов у бюджета и "
